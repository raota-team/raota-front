...
Surface treatment is disciplined and predictable: a three-surface pass of white (editorial canvas) → Vodafone red (CTA buttons, selected states, the famous speech-mark logo, and occasional chapter dividers) → near-black charcoal (footer, share-ticker panel, global-impact map). There is almost no decorative shadow, almost no gradient, and almost no rounded-corner softness. Edges are small and clinical (2px and 6px), buttons operate as a two-tier system — tight 2px rectangles for utility/form actions, and fully-rounded 60px pills for primary content CTAs. This is a design system that trusts the brand color to do the heavy lifting and gets out of its way everywhere else.

**Key Characteristics:**
- Vodafone Red (`#e60000`) is the single dominant accent — used for CTAs, selected states, optional dividers, the speech-mark logo, and the rotated "IMPACT" brand-mark type on the sustainability map
- Monumental uppercase display type (up to 144px, weight 800, negative letter-spacing) paired with calm 16-18px body copy
- A universal page rhythm: dark atmospheric hero → monumental uppercase headline → white editorial canvas → dark charcoal institutional panel → charcoal footer. Red bands are optional chapter breaks, not mandatory after every hero
...
- **Surface Red Band** (`#e60000`): The same brand red can be deployed as a full-width band between major editorial sections — functions as a chapter divider and a visual amplifier for the brand. Use sparingly; do not place it automatically under every page hero.
...
RAOTA's home page should feel like a ramen discovery dashboard, not a marketing-only landing page. Lead with search, curated recommendations, and recent verified activity.
...
- Recent activity: prefer image-led cards for recently verified shops so the home page feels alive and food-specific

### Community Post Detail

Community post detail pages are reading surfaces, not brand hero moments. The post title should lead the article without overpowering author metadata, images, or the body.

- Title scale: use a compact article title around `21px-24px` on mobile and about `30px` on desktop. Avoid hero-scale `48px+` titles inside the article card
- Title rhythm: keep line-height relaxed enough for Korean wrapping (`1.3-1.4`) and use normal letter spacing
- Surface hierarchy: category/store chips and author metadata should remain close to the title so the page reads as one article header, not a separate landing section

### AI Taste Search Panel

RAOTA's shop-list AI taste search is a compact utility panel, not a hero. It should help users narrow the current list without making the page feel like a second landing page.
...
- Primary action: use the Primary Red Rectangle treatment, 48px high, compact Korean label such as "찾아보기", and a right arrow icon. The disabled state may use Disabled Grey (`#bebebe`) or `stone-300`
- Reset action: ghost rectangle with a reset icon; only show it when an AI condition is active
- Quick chips: `2px` radius outlined buttons, 12px bold text, white default state, solid Vodafone Red (`#e60000`) selected state. Treat them as a single-select category row: selecting one chip clears the previous chip and writes that chip label into the input; editing or clearing the input clears the chip selection. On mobile, keep them in a horizontal scroll row; from tablet up they may wrap
- Example prompts: quiet filled neutral chips using Light Neutral (`#f2f2f2`) with grey text; they should read as suggestions, not primary filters. Keep the full example set in a horizontal scroll row on mobile
- Active summary: do not repeat the active AI query inside the panel intro. Show the applied query only once in the shop-list filter chip row below the controls

### Shop Detail Compare Panel

The shop-detail 1:1 compare panel is a secondary action at the bottom of a detail page, so it should feel compact and utilitarian rather than like a full recommendation workflow.

- Mobile shop-detail content: when the main and side columns collapse into one column, keep the column gap compact (`16px`) and use a shorter bottom margin after the regular menu section (`24px`) so the ramen-log preview reads as the next related section rather than a distant block
- Mobile shop-detail top rhythm: keep the back navigation close to the app header and keep only about `20px` between the back action and hero image; do not let the back link float in a large blank band
- Container: white surface, 1px neutral border, `2px` radius, no shadow, no gradient
- Header: compact eyebrow with chart icon, 16-18px title, and a small "기준: {shop name}" summary chip instead of a full comparison-A card
- Controls: use compact dropdown/input heights around 44px on mobile and keep vertical gaps around 12px
...
### Optional Red Divider Band

A reusable component for major chapter changes only: a full-width band of Vodafone Red (`#e60000`) that runs horizontally across the page to separate large editorial modules. It carries no text and no controls — it simply is the brand's way of saying "new chapter." Typical height: 40-80px. For RAOTA, do not use this band directly below the home, shop-list, community, or recommendation page heroes; let the hero transition into the white editorial canvas with spacing instead.
...
Vodafone's editorial canvas leans generous — whitespace is used as a visual palette cleanser between a monumental headline and the card grid or data panel that follows. Sections are separated by tall vertical rhythm (64-96px), with an occasional red band reserved for deliberate chapter transitions. Within cards, spacing is tight and efficient (12-16px) so the photography can take the stage.
...
- Use Vodafone Red (`#e60000`) as the single loudest element on any screen — one primary CTA per fold, with red bands reserved for intentional editorial breaks
...
- Use red bands only when a page needs a strong chapter divider; do not automatically put one between every hero and the content below it
...
- Respect the universal page rhythm: dark hero → white editorial → charcoal institutional → charcoal footer, with optional red chapter dividers only where they add clarity
...
- **Optional red divider bands**: when used, remain full-width at every breakpoint; their vertical height compresses from ~80px at desktop to ~40px at mobile
...
- "Create an optional red divider band only for a major chapter change: full-width strip of Vodafone Red (`#e60000`), 64px tall on desktop and 40px on mobile, no text, no controls. Do not place it directly below RAOTA page heroes by default."
