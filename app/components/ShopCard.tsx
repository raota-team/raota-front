...
const ShopCard: React.FC<ShopCardProps> = ({ shop, getTotalVotes }) => {
  return (
    <Link
      href={`/shop/${shop.id}`}
    ...
    >
      <div className="aspect-w-16 aspect-h-9 h-48 overflow-hidden relative">
        <img src={shop.imageUrl} alt={shop.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.03]" />
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex items-center justify-between text-xs text-stone-400 font-mono border-t border-stone-200 pt-4 mt-auto">
          ...
          <div className="flex items-center"><BarChart2 className="w-3 h-3 mr-1" />Votes: {getTotalVotes(shop)}</div>
        </div>
      </div>
    </Link>
  );
