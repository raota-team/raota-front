...
export default function ShopDetailPage() {
  ...
  const totalVotes = getTotalVotes(shop);
  const sortedMenus = [...shop.menus].map((menu, index) => ({ ...menu, originalIndex: index })).sort((a, b) => b.votes - a.votes);
        ...
        <div className="lg:col-span-8">
          <div className="relative h-80 lg:h-96 w-full mb-6 overflow-hidden rounded-lg group">
            ...
            <div className="absolute bottom-0 left-0 p-8">
              <span className="inline-block px-3 py-1 bg-red-600 text-white text-xs font-bold tracking-widest mb-3">에디터 픽</span>
              ...
              <div className="flex items-center text-stone-200 space-x-4 text-sm font-mono">
                ...
                <span className="flex items-center text-yellow-400 font-bold"><Camera className="w-4 h-4 mr-1" /> 인증 {shop.userPhotos?.length || 0}회</span>
              </div>
            </div>
          </div>

          <div className="prose prose-stone max-w-none mb-12">
            <h3 className="text-xl font-bold text-stone-900 mb-4 border-l-4 border-red-600 pl-4">에디터 리뷰</h3>
          ...
          {shop.event_menus && shop.event_menus.length > 0 && (
            <div className="mb-12">
              <h3 className="text-xl font-bold text-stone-900 mb-6 flex items-center">
                <Sparkles className="w-5 h-5 mr-2 text-yellow-500" fill="currentColor" />
                스페셜 이벤트 메뉴
              </h3>
                  ...
                  <div key={event.id} className="bg-white border border-stone-200 rounded-lg overflow-hidden flex flex-col md:flex-row group hover:border-pink-300 transition-colors shadow-sm">
                    <div className="md:w-1/3 h-48 md:h-auto overflow-hidden relative">
                      ...
                      <div className="absolute top-2 left-2">
                        <span className="bg-pink-600 text-white text-[10px] font-bold px-2 py-1 rounded-sm uppercase tracking-wider">{event.badge_text}</span>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center">
                        <div className="w-12 h-12 rounded-sm overflow-hidden mr-4 bg-stone-100 flex-shrink-0 relative">
                          ...
                          {isActive && <div className="absolute inset-0 bg-red-600/20 flex items-center justify-center"><Filter className="w-4 h-4 text-red-600" /></div>}
                        </div>
                        <div>
                          <div className="flex items-center">
                            <span className={`font-bold mr-2 ${isActive ? 'text-red-500' : 'text-stone-900'}`}>{menu.name}</span>
                            {menu.is_signature && <span className="bg-red-600 text-white text-[10px] px-1.5 py-0.5 rounded-sm font-bold uppercase tracking-tighter">SIG</span>}
                          </div>
                          <span className="text-xs text-stone-400 hidden sm:inline-block">클릭하면 사진 보기</span>
                        </div>
                      </div>
          ...
          <div className="mt-12 border-t border-stone-200 pt-8">
            <div className="flex justify-between items-end mb-6">
              ...
              <span className="text-xs text-stone-400 font-mono">{displayedPhotos?.length || 0}개 사진</span>
            </div>
            ...
            {displayedPhotos && displayedPhotos.length > 0 ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                >
                  ...
                  <span className="text-xs font-bold uppercase tracking-wider">사진 올리기</span>
                </div>
              </div>
            ) : (
              <div className="py-12 text-center border-2 border-dashed border-stone-300 rounded-sm bg-stone-50">
                <p className="text-stone-400 mb-2">No photos found for <span className="text-red-500 font-bold">{filterMenu}</span></p>
                <button className="text-sm text-stone-500 underline hover:text-stone-900" onClick={() => setFilterMenu(null)}>View all photos</button>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="sticky top-24 space-y-8">
            <div className="bg-white p-6 border border-stone-200 rounded-lg shadow-sm">
              <div className="flex items-center justify-between mb-6 border-b border-stone-200 pb-4">
                <div>
                  <h3 className="text-xl font-black text-stone-900 uppercase italic">베스트 메뉴 투표</h3>
                  <p className="text-xs text-stone-500 mt-1">이 가게의 필청(必聽) 메뉴는?</p>
                </div>
                ...
                {sortedMenus.map((menu) => (
                  <div key={menu.name} className="relative">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button onClick={() => handleVote(shop.id, menu.originalIndex)} className="text-xs bg-stone-100 hover:bg-red-600 text-stone-600 hover:text-white px-3 py-1 transition-colors uppercase font-bold tracking-wider">투표</button>
                    </div>
            ...
            <div className="bg-white p-6 border border-stone-200 rounded-lg shadow-sm">
              <h4 className="text-sm font-bold text-stone-500 mb-4 uppercase tracking-widest">가게 정보</h4>
              <div className="space-y-3 text-sm text-stone-700">
                <p className="flex justify-between border-b border-stone-200 pb-2"><span className="text-stone-400">영업시간</span><span className="font-mono text-stone-900">{shop.business_hours?.open_time} - {shop.business_hours?.close_time}</span></p>
                {shop.business_hours?.break_start && <p className="flex justify-between border-b border-stone-200 pb-2"><span className="text-stone-400">브레이크 타임</span><span className="font-mono text-stone-900">{shop.business_hours?.break_start} - {shop.business_hours?.break_end}</span></p>}
                <p className="flex justify-between border-b border-stone-200 pb-2"><span className="text-stone-400">휴무일</span><span className="text-stone-900">{shop.business_hours?.closed_days}</span></p>
                <p className="flex justify-between border-b border-stone-200 pb-2"><span className="text-stone-400">주차</span><span className="text-stone-900">불가</span></p>
                {shop.instagram_url && (
                  <div className="pt-2"><a href={shop.instagram_url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full bg-stone-100 hover:bg-gradient-to-r hover:from-purple-500 hover:to-pink-500 text-stone-600 hover:text-white py-2 rounded-sm transition-all text-xs font-bold uppercase tracking-wider"><Instagram className="w-4 h-4 mr-2" /> 공식 인스타그램 <ExternalLink className="w-3 h-3 ml-2 opacity-50" /></a></div>
                )}
                <div className="pt-2"><a href="https://app.catchtable.co.kr" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full bg-stone-100 hover:bg-orange-500 text-stone-600 hover:text-white py-2 rounded-sm transition-all text-xs font-bold uppercase tracking-wider"><Utensils className="w-4 h-4 mr-2" /> 캐치테이블 예약 <ExternalLink className="w-3 h-3 ml-2 opacity-50" /></a></div>
